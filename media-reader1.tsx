...
const MediaReader = () => {
  ...
  const handleFileUpload = async (event) => {
      ...
      if (file.type === 'text/plain') {
        const promise = new Promise((resolve) => {
          ...
          reader.onload = (e) => {
            newFiles[file.name] = {
              type: 'text',
              content: e.target.result // Store the full text content
            };
            resolve();
          };
    ...
    // Update state only once with all files
    setFiles(newFiles);
  ...
  const goToPreviousText = useCallback(() => {
    const currentIndex = sortedFiles.text.indexOf(currentTextFile);
    // If at the beginning or no current file, go to end
    if (currentIndex <= 0) {
  ...
  const handleKeyPress = useCallback((event) => {
    ...
    switch(event.key) {
      case 'ArrowRight':
        // Right arrow only controls text files (top section)
        event.preventDefault();
        goToNextText();
        break;
      case 'ArrowLeft':
        // Left arrow only controls image files (bottom section)
        event.preventDefault();
        goToNextImage();
        break;
      default:
          ...
          <CardContent>
            {sortedFiles.text.length > 0 ? (
              <>
                ...
                {currentTextFile && (
                  <div className="prose max-w-none whitespace-pre-wrap overflow-y-auto max-h-[calc(100vh-400px)]">
